<x-layout>
    <div class="container mx-auto">
        <div class="my-5">
            <div class="font-light text-3xl font-normal">Edit This Todo</div>
        ...
        <div class="todo_create my-5">
            <x-form class="w-full max-w-lg" method="POST" action="{{ route('put.update.todo',$todo->id) }}">
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 md:mb-0">
                        <x-input.group label="Name todo" for="name" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">
                            <input type="text" placeholder="Write here..." id="name" name="name" value="{{$todo->name}}" class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" require @if(old("name")) value="{{old('name')}}" @endif />
                        </x-input.group>
                        @error('name')
                    ...
                    <div class="w-full md:w-1/2 px-3">
                        ...
                        <div class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white">
                            <select id="tags" name="tags" class="w-full bg-transparent outline-none">
                                <option value="work" @if($todo->tags == 'Work') selected @endif>Work</option>
                                <option value="study" @if($todo->tags == 'Study') selected @endif >Study</option>
                                <option value="personal" @if($todo->tags == 'Personal') selected @endif >Personal</option>
                                <option value="other" @if($todo->tags == 'Other') selected @endif >Other</option>
                            </select>
                        </div>
                        @error('tags')
                        <div class="alert alert-danger text-rose-500 mt-2 text-xs">{{ $message }}</div>
                ...
                <div class="w-full mb-7">
                    <x-input.group label="Status" for="status" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"></x-input.group>
                    <div class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white">
                        <select id="status" name="status" class="w-full bg-transparent outline-none">
                ...
                <div class="w-full mb-7">
                    <x-input.group label="Starte Date" for="start_date" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">
                        <input id="start_date" name="start_date" type="date" class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white" placeholder="Select date" require @if(old("start_date")) value="{{ old('start_date')}}" @endif value="{{$todo->start_date}}" />
                    </x-input.group>
                    @error('start_date')
                    <div class="alert alert-danger text-rose-500 mt-2 text-xs">{{ $message }}</div>
                    @enderror
                ...
                <div class="w-full mb-7">
                    ...
                    <x-input.group label="Note" for="note" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">
                        <input type="text" placeholder="Write here..." id="note" value="{{$todo->note}}" name="note" class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" @if(old("name")) value="{{old('name')}}" @endif />
                    </x-input.group>
                ...
                <div class="w-full mb-7">
                    ...
                    <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
                        ...
                        <textarea id="des" name="des" rows="8" class="appearance-none block w-full bg-gray-200 text-gray-700 py-3 px-4 leading-tight focus:outline-none focus:bg-white" placeholder="Write an article...">@if(old("des")) {{old('des')}} @endif {{$todo->des}}</textarea>
                    </div>
